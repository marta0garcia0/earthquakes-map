...
test('details render', async() => {
	...
	const route = '/detail/pr2021097004';
	...
	await act(async () => {
 	...
 	const container = detail.container.querySelector('.detail-info');
	expect(container).toHaveTextContent('Title:');
	expect(container).toHaveTextContent('M 2.4 - 4 km SE of Maria Antonia, Puerto Rico');
});

test('details redirect home when wrong id', async() => {
	jest.spyOn(mapbox, 'Map').mockImplementation(() => {
		return {
			setCenter: () => {},
			setZoom: () => {},
	...
	jest.spyOn(api, 'getEarthQuakeById').mockImplementation(() => Promise.reject());
